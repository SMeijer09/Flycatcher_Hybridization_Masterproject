#test the effect of measurer on phenotypic measurements
library(tidyverse)
library(lme4)
library(lmerTest)
library(patchwork)
...
data <- read.csv("/Users/semmeijer/Downloads/Ecology&Conservation/Flycatcher_Hybridization/Data/database_preferences_updated_08.26.csv") |>
  ...
  select(-X, -recruitnb_new, -total_mass_d12_new, -mean_mass_d12_new, -fledge_nb_new, -predation, -predation_date, -nb_eggs, -experiment) |> #filter for nestbox with AVI in the nestbox code
...
model_data <- male_data |> select(yearAreaBox, year, measurer_m, nestbox,n_birds, tail_m, tarsus_m, wing_m, mass_m, beak_m, patch_size_m, sum_of_white_on_primaries_m, adj.wing_patch_m, adj.patch_size_m)


m1 <- lmer(tarsus_m ~ measurer_m + (1 | year), data = model_data)
summary(m1)
m2 <- lmer(tail_m ~ measurer_m + (1 | year), data = model_data)
summary(m2)
m3 <- lmer(wing_m ~ measurer_m + (1 | year), data = model_data)
summary(m3)
m4 <- lmer(patch_size_m ~ measurer_m + (1 | year), data = model_data)
summary(m4)
m5 <- lmer(sum_of_white_on_primaries_m ~ measurer_m + (1 | year), data = model_data)
summary(m5)
anova(m1)
anova(m2)
anova(m3)
